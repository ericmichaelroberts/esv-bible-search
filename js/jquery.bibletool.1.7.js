...
(function(parameters){
	...
	var Total=Patterns.Books.length;
	var Results=[];
	var $Wrapper='#PopupWrapper',$Mask,$Content,$Close,$Trigger,$Form,$Field='.BibleSearchField';
		...
		init: function(options){
			...
			if(Settings.wrapclass||Settings.wraplink||Settings.reformat){
				$(this).each(function(ind,ele){
					var mrkp = $(this).html();
					$(this).html(mrkp.replace(/\s{2,}/gi,' '));
					var txt=$(this).text(),test=Patterns.Hit.test(txt),hits;
					if(test){
						hits=hits_to_objects(txt.match(Patterns.Hit));
						if(Settings.logging){log_hits(hits);}
			...
			if(Settings.addSearch){
				var inputTxt;
				$Field=(parseInt($($Field).size())>0)?'.'+get_unused_class($Field):$Field;				
	...
	var show_scripture = function(url){
	...
	var validate = function(obj){
		obj.rangeString=new String(obj.orig.match(Patterns.Range).pop()).trim();
		obj.bookString=obj.orig.slice(0,(obj.orig).indexOf(obj.rangeString) - 1);
	...
	var validate_book= function(obj){
		for(var s=0;s<Total;s++){
			if(Patterns.Books[s][0].test(obj.bookString)){
				obj.bookIndex = s;
				return true;}}
		obj.valid=false;
	...
	var validate_chapter = function(obj){
		obj.chapterCeil=Patterns.Books[obj.bookIndex][2];
		build_ranges(obj);
	...
	var validate_verse = function(obj){
		for(var x=0;x<obj.verseRange.length;x++){
			chap=obj.verseRange[x][0];
			maxVerse=highest_verse(obj.verseRange[x][1]);
			if(maxVerse>Patterns.Books[obj.bookIndex][3][chap-1]){
				obj.valid=false;
				obj.FailedAt='VerseValidation';}}
		return true;
	}
	
	var highest_verse = function(string){
		return Math.max.apply(null,integers(string.replace(/[^\d]/g,' ').replace(/\s+/g,' ').trim().split(' ')));
	}
	
	var build_ranges = function(obj){
		var verses=[],x,ys,y,struct=(('<('+(obj.rangeString).replace(/\b(\d+)[:]/g,')><$1(')+')>').replace(/([,]\)|\<\(\)\>)/g,'')).replace(/(\-\)\>\<)/g,')>-<').replace(/(\>\<)/g,')X').replace(/[<>]/g,'').replace(/[)][-]/g,')X').replace(/(\-)/g,' - ').split('X'),t=struct.length;
		obj.maxChap=0;
		for(x=0;x<t;x++){
			if(/^[(]/.test(struct[x]))y=Math.max.apply(null,(((struct[x].replace(/[()]/g,'')).replace(/[^0-9]/g,' ')).replace(/\s+/g,' ')).split(' '))
			else{
				ys=struct[x].match(/^(\d+)\(/g);
				y=((typeof ys==='object')&&(ys instanceof Array)&&(ys.length>1))?ys.slice(1).max:parseInt(ys);}
			obj.maxChap=(parseInt(y)>obj.maxChap)?parseInt(y):obj.maxChap;
			verses.push([y,struct[x].substring((struct[x].indexOf('(')+1),struct[x].lastIndexOf(')'))]);}
		obj.verseRange=verses;
	}
	
	var rewrite = function(obj){
		var string=obj.orig,tagname=(Settings.wraplink)?'a':'span',atts=[];
		...
		var text=(Settings.reformat)?reformat(string):string;
		var tag='<'+[tagname].concat(atts).join(' ')+'>'+text+'</'+tagname+'>';
		obj.rewrote=tag;
	}
	
	var integers = function(input){
		var output=new Array;
		while(input.length>0)	output.push(parseInt(input.shift()));
		return output;
	}
	
	var hits_to_objects = function(input){
		var input=(input instanceof Array)?input:[input];
		var output=new Array;
		while(input.length>0)	output.push({orig:input.shift(),valid:true});
		return output;
	}

	var log_hits = function(input){
		console.log('Hits: '+input);
	}
	
	var log_replacement = function(input,output){
		console.log('Replacement: "'+input+'" ---> "'+output+'"');
	}
	...
	var build_close = function(){
		return '<a class="'+($Close.split('.').pop())+'" title="Close" style="position:absolute;right:-18px;top:-18px;" href="#"><img src="http://why.jesus.net/WJ-BGEA-USA/file.php/2/Images_and_Graphics/closebtn.png" width="36px" height="36px" style="border:none;" /></a>';
	}
	...
	var build_href = function(string){
		return 'http://www.esvapi.org/v2/rest/passageQuery?key=IP&passage='+(string.replace(/\s+/g,'+'));
	}
	
	var get_unused_class = function(string){
		var x=0, y='', string=(string[0]=='.')?string.slice(1):string, testclass=string;
		while(!(parseInt($('.'+testclass).size)==0) && x < Settings.recursionLimit){testclass=string+((x==0)?'':'_'+(++x));}
		return testclass;
	}
	
	var get_unique_id = function(string){
		var x=0, y='', string=(string[0]=='#')?string.slice(1):string, testid=string;		
		while(!(parseInt($('#'+testid).size)==0) && x < Settings.recursionLimit){testid=string+((x==0)?'':'_'+(++x));}
		return testid;
	}
	
	var initialize_ids = function(){
