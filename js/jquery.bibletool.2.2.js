...
(function($){
		...
		init: function(options){
			...
			if(Settings.wraplink){
				...
				$('a.'+Settings.wrapclass).on({
					...
					mouseleave: function(e){hide_tooltip(e.currentTarget);}
				});
			}
	...
	rewrite = function(obj){
		...
		if(Settings.wraplink!=false){
			...
			//atts.push('title="please wait..."');
		}
		var text=(Settings.reformat)
			?	reformat(string)
			:	string;
		var tag='<'+[tagname].concat(atts).join(' ')+'><div class="tooltip init"></div>'+text+'</'+tagname+'>';
		obj.rewrote=tag;
	},
	...
	build_wrapper = function(){
		...
		return tag('div',obj,build_mask()+build_content());
	},

	build_mask = function(){
		var obj = {'class': ($Mask.split('.').pop()),style:{position:'absolute',top:'0px',left:'0px',width:($(document).width())+'px',height:($(document).height())+'px','background-color':'#888888',opacity:.5,filter:'alpha(Opacity=80)'}};
		return tag('div',obj);
	},

	build_content = function(){
		var obj = {'class':($Content.split('.').pop()),style:{display:'inline-block','background-color':'white',position:'relative','text-align':'left'}};
		return tag('div',obj);
	},
