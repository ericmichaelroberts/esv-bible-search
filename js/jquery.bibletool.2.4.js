...
(function($){
		...
		init: function(options){
			...
			if(Settings.wraplink){
				...
				$($Close+','+$Mask).on({
					click: function(e){
						e.preventDefault();
						$($Close).animate({opacity:0},250,'swing',function(){
	...
	//Internals
	refresh_dims = function(){
		Dims.Ww= $(window).width();
		Dims.Wh= $(window).height();
		Dims.Dw= parseInt(maxVal(numbers([$(document).width,$('body').innerWidth()],true)));
		Dims.Dh= parseInt(maxVal(numbers([$(document).height,$('body').innerHeight(),document.body.clientHeight,document.body.scrollHeight],true)));
	},
	...
	center_div_new = function(div){
		...
		$(div).css({width: Dims.dW+'px',height:'0px',top:Dims.dY+'px',left:Dims.dX+'px',padding:'0px'});
		$(div).animate({height: $('.esv').outerHeight()+'px','padding-bottom':10,'padding-top':10},450,'swing',function(){
			//$($Close).css({visibility: 'visible',opacity:0}).animate({opacity:1},250,'swing');
		});
		if(Settings.logging){console.log(Dims);}
	},
	
	center_div = function(div){
		var dims = {};
		var p = $(div).parent();
		dims.pW = $(window).width();
		dims.pH = $(window).height();
		var capHeight = dims.pH - 20;
		dims.dW = $('.esv').outerWidth();
		if(dims.dW > (dims.pW * .75)) {dims.dW = parseInt(dims.pW * .75);}
		dims.dH = $('.esv').outerHeight()+5;
		if(dims.dH > (dims.pH * .75)) {dims.dH = parseInt(dims.pH * .75);}
		dims.dX = parseInt((dims.pW * .5) - (dims.dW * .5));
		dims.dY = parseInt((dims.pH * .25) - (dims.dH * .5));
		$($Close).css('visibility','hidden');
		$(div).css({width: dims.dW+'px',height:'0px',top:dims.dY+'px',left:dims.dX+'px',padding:'0px'});
		$(div).animate({height: $('.esv').outerHeight()+'px','padding-bottom':10,'padding-top':10},450,'swing',function(){
			$($Close).css({visibility: 'visible',opacity:0}).animate({opacity:1},250,'swing');
	...
	build_wrapper = function(){
		var obj = {id:($Wrapper.slice(1)),style:{visibility:'hidden',position:'fixed',top:'0px',left:'0px',width:$(document).width()+'px',height:$(document).height()+'px'}};
		return tag('div',obj,build_mask()+build_content()+build_close());
	},
	
	build_mask = function(){
		var obj = {'class':($Mask.split('.').pop()),style:$.extend({position:'absolute',top:'0px',left:'0px',width:($(document).width())+'px',height:($(document).height())+'px'},Settings.style.mask)};
		return tag('div',obj);
	},
	
	build_content = function(){
		var obj = {'class':($Content.split('.').pop())};
		return tag('div',obj);
	},
	
	build_close_img = function(){
		var obj = {src: Settings.closeButton.src,width:'36px',height:'36px',style:{border:'none'}};
		return tag('img',obj);
	},
	
	build_close = function(){
		var obj = {'class':($Close.split('.').pop()),title:'Close',href:'#',style:{position:'absolute',right:'-18px',top:'-18px'}};
		return tag('a',obj,build_close_img());
	},
	...
	show_scripture = function(url){
		if(Settings.ajax){return show_ajax_scripture(url);}
			...
			var markup=build_close()+(tag('iframe',{src:url,width:'100%',height:'100%',frameborder:0,scrolling:'auto'}));
			$($Content).html(markup);
	...
	adjust_contents = function(){
		var temp = {
			Ww: $(window).width(),
			Wh: $(window).height(),
			Dw: parseInt(maxVal(numbers([$(document).width,$('body').innerWidth()],true))),
			Dh: parseInt(maxVal(numbers([$(document).height,$('body').innerHeight(),document.body.clientHeight,document.body.scrollHeight],true))),
			ContentW: $($Content).outerWidth(),
			ContentH: $($Content).outerHeight(),
			ContentY: parseInt($($Content).css('top'))
		};
		var css = {top: 25+'px'};		
		...
		if(temp.ContentY < 15){$($Content).animate(css,750,'swing');}
		console.log(temp);
	},
	
	show_ajax_scripture = function(url){
		var widthLimit=	parseInt($($Wrapper).innerWidth()*.8);
		var heightLimit=	parseInt($($Wrapper).innerHeight()*.8);
		...
		var cback = function(content){
			...
			$($Content).html(markup);
			center_div($Content);
			$($Wrapper).css({visibility:'visible',opacity:0}).animate({opacity:1},250,'swing',function(){adjust_contents();});
		};
		...
		debug.screenHeight 	= $(window).height();
		debug.screenWidth		=	$(window).width();
		debug.contentHeight	= $('.PopupContent').innerHeight();
		debug.contentWidth	= $('.PopupContent').innerWidth();
		debug.contentTop		= $('.PopupContent').innerHeight();
	...
	show_ajax_scripture_bak = function(url){
		var widthLimit=	parseInt($($Wrapper).innerWidth()*.8);
		var heightLimit=	parseInt($($Wrapper).innerHeight()*.8);
		var cback = function(content){
			var markup=build_close()+content;
			...
			$($Close+','+$Mask).click(function(e){
				e.preventDefault();
				$($Close).animate({opacity:0},250,'swing',function(){
					$($Wrapper).animate({opacity:0},250,'swing',function(){$($Wrapper).css({visibility:'hidden'});});
				});
			});
		};
		call_ajax(url,'html',5000,cback);
	...
	show_tooltip = function(a){
		var div = $(a).contents('span'),
				href = $(a).attr('href'),
				pos = $(a).offset(),
				height = $(a).innerHeight();								
		div.css({top:parseInt(pos.top + height + 2),left:parseInt(pos.left)});		
		if(div.hasClass('init')){
			div.html(Settings.tooltipWaitMsg);
				call_ajax(href,'text',5000,function(cont){
					clearInterval(div.data('timer'));
					div.removeClass('waiting').html(cont);
				});
				div.animate({opacity:1},250,'swing',function(){
					div.addClass('waiting').removeClass('init');
					div.data('timer',setInterval(function(){
						if(div.hasClass('waiting')){
							var h = div.html();
							div.html(h==Settings.tooltipWaitMsg+'...'?Settings.tooltipWaitMsg:h+'.');
						}else{
							clearInterval(div.data('timer'));
						}
					},100));
