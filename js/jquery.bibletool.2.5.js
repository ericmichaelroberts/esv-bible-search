...
(function($){
	...
	Methods={
		init: function(options){
			...
			Total = Books.length;
			inject_wrapper();
			Settings.validateChapter = Settings.validateBook
				?	Settings.validateChapter
				:	false;
			Settings.validateVerse = Settings.validateChapter
				?	Settings.validateVerse
				:	false;
	...
	show_tooltip = function(a){
		...
		if(span.hasClass('init')){
			span.html(Settings.tooltipWaitMsg);
				...
				span.animate({opacity:1},250,'swing',function(){
					...
					span.data('timer',setInterval(function(){
						if(span.hasClass('waiting')){
							var h = span.html();
							span.html(h==Settings.tooltipWaitMsg+'...'?Settings.tooltipWaitMsg:h+'.');
						}else{
							clearInterval(span.data('timer'));
						}
					},100));
