...
(function($){
	...
	Methods={
		init: function(options){
			...
			if(Settings.wraplink){
				...
				$('a.'+Settings.wrapclass).mouseenter(function(e){
					$(this).addClass('tooltip');
					$(this).mouseleave(function(e){$(this).removeClass('tooltip');});
					if(['undefined',null,false,''].indexOf($(this).attr('title'))>0){
						call_ajax($(this).attr('href'),'text',2000,function(x){$('a.tooltip').attr('title',x);});
					}
				});
	...
	build_content = function(){
		var obj = {class:($Content.split('.').pop()),style:{display:'inline-block','background-color':'white',position:'relative',border:'2px solid black','border-radius':'8px','text-align':'left'}};
		return tag('div',obj);
	},
